...
London_pop_grouped_ed <- London_pop_grouped %>%
...
London_pop_grouped_line <- London_pop_grouped_ed %>%
 ...
 # scale_alpha_manual( values = c(NA, 1)) +
  ...
  scale_x_continuous(limits = c (1993, 2023), breaks = c(1993, 1997, 2001, 2005, 2009, 2013, 2017, 2021)) + 
...
ggsave (filename = (paste0(chart_dir, "London_pop_grouped_line.png")),
...
London_pop_grouped_ind_line_1993 <- London_pop_grouped_ind %>%
 ...
 # theme(legend.position = "right") +
  ...
  labs(title= "Population in London by age group, indexed to 1993, 1993 - 2021", 
       caption = paste0("Source: ONS, Chart: GLA demography"))
London_pop_grouped_ind_line_1993
...
London_pop_grouped_ind_2001 <- London_pop_grouped %>%
  group_by(agegroups) %>% 
  filter(year > 2000) %>%
  mutate(indexed_pop = (Sum_value/first(Sum_value))*100) %>% 
  mutate(label = if_else(year == max(year), as.character(agegroups),NA_character_)) %>%
...
London_pop_grouped_ind_line_2001 <- London_pop_grouped_ind_2001 %>%
  ggplot(aes(x = year, y = indexed_pop, group = agegroups, color = agegroups, text = paste("Year: ", year,
                         "<br>Age group: ", agegroups,
                         "<br>Indexed Population: ", round(indexed_pop, digits = 3))
)) +
 ...
 #  ylim(80, 160) +  
   labs(title= "Population in London by age group, indexed to 2001, 2001 - 2021", 
       caption = paste0("Source: ONS, Chart: GLA demography"))
London_pop_grouped_ind_line_2001
...
London_pop_grouped_ind_2011 <- London_pop_grouped %>%
  group_by(agegroups) %>% 
  filter(year > 2010) %>%
  mutate(indexed_pop = (Sum_value/first(Sum_value))*100) %>% 
  mutate(label = if_else(year == max(year), as.character(agegroups),NA_character_)) %>%
...
London_pop_grouped_ind_line_2011 <- London_pop_grouped_ind_2011 %>%
  ggplot(aes(x = year, y = indexed_pop, group = agegroups, color = agegroups, text = paste("Year: ", year,
                         "<br>Age group: ", agegroups,
                         "<br>Indexed Population: ", round(indexed_pop, digits = 3))
)) +
...
London_pop_zone_grouped_ind <- London_pop_zone_grouped %>%
...
London_pop_grouped_line_facet <- London_pop_zone_grouped_ind %>%
  ggplot(aes(x = year, y = indexed_pop, group = agegroups, color = agegroups)) +
  theme_gla() +
   geom_line(aes(size=highlight_flag)) +  #, alpha=highlight_flag
...
#   theme(legend.position =" none") +
  ...
  scale_y_continuous(labels = label_number(suffix = "%")) +
    scale_x_continuous(limits = c (2011, 2023), breaks = c(2011, 2013, 2015, 2017, 2019, 2021)) +  
   labs(title= "Population in London by age group, indexed to 2011, 2011 - 2021", 
       caption = paste0("Source: ONS, Chart: GLA demography")) +
...
ggsave (filename = (paste0(chart_dir, "London_pop_grouped_line_facet.png")),
...
merged_pop_tot = rbind(InnerLondon_pop_grouped, OuterLondon_pop_grouped, EandW_pop_grouped) %>%    data.frame()
