---
title: "Population, Nov 23rd and Dec 7th 2023, and Feb 2024
output: 
  html_document:
    css: Wil_style.css
    theme: default
...
change_2011to2021_depChildren_geog <- DepChildren_Change %>% 
 ...
 # mutate(Perc_Cat = cut(changePerc, breaks = c(-15, -4, -2, 2, 6, 10, 50),
  mutate(Perc_Cat = cut(changePerc, breaks = c(-20, -10, -5, -2, 4,  8, 12, 30),
                             right=FALSE,
                             labels = c("-20.0 -",
                                        "-10.0 -",
                                        "- 5.0 -",
...
change_nondepchildren  <- change_nondepchildren_geog  %>%
  ...
  theme(legend.key.height= unit(1, 'cm'),
        legend.key.width= unit(0.7, 'cm')) +
   labs(title = "Number of households with non-dependent children", 
       subtitle = "Change from Census 2011 to Census 2021",
       caption = "Source: Census 2011 & Census 2021, Chart: GLA demography") 
change_nondepchildren
 
 
ggsave (filename = (paste0(chart_dir, " change_nondepchildren_geog.png")),
...
## making a map showing the change in number of households with non-dependent children across London by Borough
