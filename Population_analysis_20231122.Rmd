---
title: "Population analysis, November 22nd 2023"
output: 
  html_document:
    css: Wil_style.css
    theme: default
...
input_2011data_long <- input_2011data %>% 
  ...
  mutate(census = 2011) %>%
  mutate(Borough_code = LA_code) %>%
  mutate(Borough_name = LA_name) %>%
  select (-c(year, Region_code, Region_name, LA_code, LA_name)) %>%
    data.frame()
...
## population pyramid with 2011 data on the left and 2021 on the right
...
```{r fig_pop_pyramid2_SYA, fig.height = 5.56, fig.width = 9,  echo=FALSE, warning=FALSE, message=FALSE}

## Do bar chart instead of population pyramid, with just and 2021 on the right



London_ageStructure_2021 <- Census_total_byLondon %>%
    mutate(census_factor = as.factor(census)) %>%
    filter(census == "2021") %>% 
  data.frame()


London_ageStructure_2011 <- Census_total_byLondon %>%
    mutate(census_factor = as.factor(census)) %>%
    filter(census == "2011") %>% 
  data.frame()
...
London_ageStructure <- London_ageStructure_2021 %>% 
    ggplot(aes(x = Age_code, y = Sum_value/1000)) + 
    theme_gla()+
    geom_bar(data = London_ageStructure_2021, aes(fill = ""), stat = "identity", fill="#943fa6") +
    geom_line(data = London_ageStructure_2011, aes(x = Age_code, y = Sum_value/1000)) +
    scale_y_continuous(labels = label_number(suffix = "K", big.mark = ","))+ # scale = 1e-3
    coord_flip() +
  theme(axis.text = element_text(size = 16)) +
    labs(title= "Age structure of London in 2021", 
         x = "Age structure of London in 2021",
         y = "Population (thousands)",
     caption = paste0("Source: ONS, Chart: GLA demography"))
London_ageStructure

ggsave (filename = (paste0(chart_dir, "London_ageStructure.png")),
         plot = London_ageStructure,
         device = "png",
         dpi=600,
         width = 9,
         height = 12,
...
VeryYoungChild_total <- London_calc %>% 
  filter(Age_code < 5) %>%
  summarise(Total_X2011 = sum(X2011), Total_X2021 = sum(X2021)) %>%
  data.frame()

YoungChild_total <- London_calc %>% 
  filter(Age_code < 11) %>%
  summarise(Total_X2011 = sum(X2011), Total_X2021 = sum(X2021)) %>%
  data.frame()

Child_total <- London_calc %>% 
  filter(Age_code < 19) %>%
  summarise(Total_X2011 = sum(X2011), Total_X2021 = sum(X2021)) %>%
  data.frame()
...
ggsave (filename = (paste0(chart_dir, "London_ageStructure_change.png")),
         plot = Census_change_byLondon2,
         device = "png",
         dpi=600,
         width = 9,
         height = 9,
...
## Import 2011 census population data for London

input_2011data <- fread(paste0(data_dir,"Census2011_London_SYA_byBorough.csv"))%>% 
  filter(grepl('E09000', LA_code)) %>%  
  data.frame()

# convert 2011 data from wide to long format and select age zero
...
## Import 2021 census population data for London and select for 10 year olds
